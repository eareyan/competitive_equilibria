...
"""
Functions to produce human readable output of objects related to markets
such as welfare-maximizing allocations and pricings. 
"""
...
def pretty_print_pricing(result):
    ...
    :return: a pretty table.
    """
    pricing_stats_ptable = PrettyTable()
    pricing_stats_ptable.title = 'Pricing'
    pricing_stats_ptable.field_names = ['Bundle', 'Price']
    if result['status'] != 'Infeasible':
        for elem in result['output_prices']:
            if elem is not None:
                for k, v in elem.items():
                    pricing_stats_ptable.add_row([k, v.varValue])
    pricing_stats_ptable.add_row(['Status', result['status']])
...
def welfare_max_stats_table(result):
    ...
    :return: a pretty table.
    """
    welfare_max_stats_ptable = PrettyTable()
    welfare_max_stats_ptable.title = 'ILP Welfare-Max. Statistics'
    welfare_max_stats_ptable.field_names = ['ilp statistic', 'value']
    for stat, units, do_format in [('generate_vars_maps', 'sec', True),
                                   ...
                                   ('status', '', False)]:
        welfare_max_stats_ptable.add_row([stat, f"{result[stat] : .4f} {units}" if do_format else f"{result[stat]}"])
    return welfare_max_stats_ptable
...
def pricing_stats_table(result):
    ...
    """
    pricing_stats_ptable = PrettyTable()
    pricing_stats_ptable.title = 'LP Pricing Statistics'
    pricing_stats_ptable.field_names = ['lp statistic', 'value']
    for stat, units, do_format in [('time_to_generate_lp', 'sec', True),
                                   ('time_to_solve_lp', 'sec', True),
                                   ('status', 'sec', False)]:
        pricing_stats_ptable.add_row([stat, f"{result[stat] : .4f} {units}" if do_format else f"{result[stat]}"])
    return pricing_stats_ptable
